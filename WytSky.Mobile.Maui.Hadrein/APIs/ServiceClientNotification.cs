...
namespace WytSky.Mobile.Maui.Hadrein.APIs
{
    public class ServiceClientNotification
    {
        ...
        #region GetAll()
        public async static System.Threading.Tasks.Task<System.Collections.ObjectModel.ObservableCollection<Dtos.StClientNotification>> GetAll(int pageIndex = 0, Dictionary<string, string> filter = null)
        {
            try
            {
                if (filter == null)
                ...
                var result = await Services.RequestProvider.Current.GetData<Models.TempletData<Dtos.StClientNotification>>(BASE, CONTROLR, filter, Enums.AuthorizationType.UserNamePassword);
