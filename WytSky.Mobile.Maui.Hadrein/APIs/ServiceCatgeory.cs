...
namespace WytSky.Mobile.Maui.Hadrein.APIs
{
    public class ServiceCatgeory
    {
        ...
        public const string CONTROLR = "StCatgeory";

        #region GetAll()
        public async static System.Threading.Tasks.Task<System.Collections.ObjectModel.ObservableCollection<Dtos.StCatgeory>> GetAll(int pageIndex = 0, Dictionary<string, string> filter = null)
        {
            try
            {
                if (filter == null)
                ...
                var result = await Services.RequestProvider.Current.GetData<Models.TempletData<Dtos.StCatgeory>>(BASE, CONTROLR, filter, Enums.AuthorizationType.UserNamePassword);
        ...
        public async static System.Threading.Tasks.Task<Dtos.ReturnData> SaveNew(Dictionary<string, object> formData)
        {
            ...
            catch (Exception ex)
            {
                string ExceptionMseeage = string.Format(" Error : {0} - {1} ", ex.Message, ex.InnerException != null ? ex.InnerException.FullMessage() : "");
                System.Diagnostics.Debug.WriteLine(ExceptionMseeage);
                ExtensionLogMethods.LogExtension(ExceptionMseeage, System.Text.Json.JsonSerializer.Serialize(formData), "ServiceCatgeory", "SaveNew()");
                return null;
            }
        }
        #endregion
        ...
        public async static System.Threading.Tasks.Task<Dtos.ReturnData> Update(Dictionary<string, object> formData)
        {
            ...
            catch (Exception ex)
            {
                string ExceptionMseeage = string.Format(" Error : {0} - {1} ", ex.Message, ex.InnerException != null ? ex.InnerException.FullMessage() : "");
                System.Diagnostics.Debug.WriteLine(ExceptionMseeage);
                ExtensionLogMethods.LogExtension(ExceptionMseeage, System.Text.Json.JsonSerializer.Serialize(formData), "ServiceCatgeory", "Update()");
                return null;
            }
        }
        #endregion
